...
export const createHMock = (): any => {
  ...
  const Rect = jest.fn().mockImplementation((top: number, left: number, bottom: number, right: number) => ({
    getTop(): number {
      return top;
    },
    getRight(): number {
  ...
  const HereMockMap = jest.fn().mockImplementation((el, baseLayer: H.map.layer.TileLayer, init = {}) => {
