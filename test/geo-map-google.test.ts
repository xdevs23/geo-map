...
test('Zoom with Google', async () => {
  ...
  const input = 1;
  const actual = await page.evaluate((factor) => TestEntry.Tests.zoomGoogle(factor), input);
...
await page.goto(`http://localhost:${ServerPort}/test?integration=true`);
  ...
  const actual = await page.evaluate((type) => TestEntry.Tests.typeGoogle(type), input);
...
await page.goto(`http://localhost:${ServerPort}/test?integration=true`);
  ...
  const screenshot = await page.screenshot({ path: './screenshots/marker-google.png' });
...
test('Viewport with Google has red marker at offset center', async () => {
  ...
  await page.goto(`http://localhost:${ServerPort}/test?integration=true`);
  await page.evaluate((left: number) => TestEntry.Tests.viewportGoogle({ top: 0, right: 0, bottom: 0, left }), offset);

  const screenshot = await page.screenshot({ path: './screenshots/viewport-google.png' });
  const parsed = await Util.parsePng(screenshot);
...
test('Traffic Layer with Google changes display', async () => {
  ...
  await page.evaluate((layer: Types.GeoLayer) => TestEntry.Tests.layerGoogle(layer), Types.GeoLayer.None);
  const before = await page.screenshot({ path: './screenshots/layer-traffic-google-before.png' });

  await page.evaluate((layer: Types.GeoLayer) => TestEntry.Tests.layerGoogle(layer), Types.GeoLayer.Traffic);
  const after = await page.screenshot({ path: './screenshots/layer-traffic-google-after.png' });
...
test('Click events with Google carry appropriate lat/lng data', async () => {
  ...
  await page.goto(`http://localhost:${ServerPort}/test?integration=true`);
  await page.evaluate((input) => TestEntry.Tests.eventPayloadGoogle(input), center);
